...
def add_post(video_info, video_metrics, SESS, BASE):

    post_url = video_info['post_url']

    TTPosts = BASE.classes.ttposts
    TTPostMetrics = BASE.classes.ttpost_metrics

    existing_post = SESS.query(TTPosts.post_url).filter(TTPosts.post_url == post_url)
    existing_post_metrics = SESS.query(TTPostMetrics.post_url).filter(TTPostMetrics.post_url == post_url)
    ...
    if not metric_count:
        ...
        SESS.add(new_post_metrics)
        SESS.commit()
        print(f"Added new post metrics: {post_url}")

def add_comments(comment_data_list, BASE, SESS):

    TTComments = BASE.classes.ttpost_comments

    id_list = [comment_data['id'] for comment_data in comment_data_list]
    post_url = comment_data_list[0]['post_url']

    existing_post_comments = SESS.query(TTComments.id).filter(TTComments.id.in_(id_list)).filter(TTComments.post_url == post_url).all()
    ...
    try:
        ...
        SESS.bulk_insert_mappings(TTComments, new_comments)
        SESS.commit()
    ...
    query = SESS.query(TTComments).filter(TTComments.post_url == post_url)
    print(f"New total # of comments for post {query.count()}")


def checkout_user(username, scrape_end, SESS, BASE):
    TTUsers = BASE.classes.ttusers
    SESS.query(TTUsers).filter(TTUsers.username == username).update({
        TTUsers.pulling_data_last_started: scrape_end,
    })
    SESS.commit()

def update_user_and_metrics(username, metrics, scrape_end, SESS, BASE, accountDeleted=False, accountPrivate=False):
    TTUsers = BASE.classes.ttusers
    TTUserMetrics = BASE.classes.ttuser_metrics
    
    current_time = datetime.utcnow()
    
    # If account is deleted, update does_not_exist column
    if accountDeleted:
        SESS.query(TTUsers).filter(TTUsers.username == username).update({TTUsers.does_not_exist: True})
        SESS.commit()
        return
    if accountPrivate:
        SESS.query(TTUsers).filter(TTUsers.username == username).update({TTUsers.is_private: True})
        SESS.commit()
        return
    ...
    existing_metric = SESS.query(TTUserMetrics).filter_by(username=username, date_collected=current_time).first()
    ...
    if existing_metric:
        ...
        SESS.add(new_metrics)

    SESS.commit()
    

def add_post_attempt(num_comments_scraped, attempt_num, post_url, SESS, BASE):
    TTPostAttempts = BASE.classes.ttpost_attempts
